<div class="container">
    ...
    <a href="<?= base_url() ?>" class="btn btn-sm btn-info mt-5"><i class="fa fa-arrow-left"></i> Back</a>
    ...
    <?php if($page->title == 'Create Post') echo form_open('posts/add', array("class" => "mt-3 needs-validation")); ?>
        ...
        <?php if($page->title == 'Edit Post'): ?> 
            <div class="d-flex justify-content-between">
                <button type="submit" class="btn btn-success mb-3" name="action" value="save_post" ><i class="fa fa-save"></i> Save Post</button>
                <a href="<?= base_url() ?>" class="btn btn-secondary"><i class="fa fa-times"></i> Cancel</a> 
            </div>
        <?php endif; ?>
        <?php if($page->title == 'Delete Post'): ?> 
            <div class="d-flex justify-content-between">
                <div>
                    <button type="submit" class="btn btn-warning mb-3" name="action" value="delete"><i class="fa fa-trash"></i> Delete</button>
                    <a href="<?= base_url() ?>" class="btn btn-secondary"><i class="fa fa-times"></i> Cancel</a>
                </div>
                <button type="submit" class="btn btn-danger mb-3" name="action" value="hard_delete"><i class="fa fa-recycle"></i> Hard Delete</button>
            </div>
        <?php endif; ?>
